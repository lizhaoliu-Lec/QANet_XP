...
class Model(object):
    ...
    def _decode(self):
        ...
        if self.config.use_position_attn:
            logits = tf.squeeze(
                conv(self._attention(tf.concat([self.enc[1], self.enc[2]], axis=-1), name="attn_logits"),
                     1, bias=True,
                     name="start_pointer"), -1)
        else:
            logits = tf.squeeze(
                conv(tf.concat([self.enc[1], self.enc[2]], axis=-1), 1, bias=True, name="logits"),
                -1)

        self.logits = mask_logits(logits, mask=tf.reshape(self.c_mask, [N, -1])),
        self.logits = tf.nn.relu(self.logits)

        self.logits = tf.layers.dense(self.logits,
                                      self.max_a_len,
                                      use_bias=True,
                                      kernel_regularizer=tf.keras.regularizers.l2(self.config.weight_decay),
                                      activation=None,
    ...
    def _compute_loss(self):
        ...
        label = tf.one_hot(self.label, tf.shape(self.logits)[1], axis=1)
